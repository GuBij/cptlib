# cptlib
This Python library facilitates the analysis of cone penetration tests (soil layers). In particular, all layers of a probe can be easily extracted and the soil behavior types are determined based on the Robertson method (2010)*. Moreover, the necessary functionality is provided such that all probes within a given geographical area, by means of a polygon, can be retrieved from the DOV (Databank Ondergrond Vlaanderen, or Database Underground Flanders).
